...
function prepare_docker_args_debian() {
    DOCKER_ARGS="--privileged --net=host \
        --name "$CONTAINER_NAME" \
        --ipc=host \
        --device /dev/dri:/dev/dri \
...
function run_docker() {
    ...
    if [ "$RESUME_CONTAINER" = true ]; then
        ...
        if [[ $os_distribution = "ubuntu" ]]; then
            prepare_docker_args_ubuntu
        elif [[ $os_distribution = "debian" ]]; then
            prepare_docker_args_debian
        else
            echo "OS: $os_distribution is not supported"
            exit 1
        fi

        if [[ $HAILORT_ENABLE_SERVICE = "true" ]]; then
...
if [ "$TAPPAS_IMAGE" != "" ]; then
    ...
    # create new imagge based of TAPPAS image with addition of required kernel headers
    docker build . -f dockerfile.tappas_run -t hailo_tappas:${CONTAINER_TAG} --build-arg=BUILD_NUMBER=${CONTAINER_TAG}
